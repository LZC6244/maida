...
class StatsMailer(object):
    ...
    def __init__(self, stats, recipients, mail, project_name, x_priority):
        # 新增 project_name 字段，发送邮件时带上项目名，便于区分爬虫
        # 新增 x_priority 字段，设置邮件优先级
        self.stats = stats
    ...
    def from_crawler(cls, crawler):
        ...
        x_priority = crawler.settings.get('X_PRIORITY', '3')
        if not recipients:
            raise NotConfigured
        mail = MailSender.from_settings(crawler.settings)
        o = cls(crawler.stats, recipients, mail, project_name, x_priority)
